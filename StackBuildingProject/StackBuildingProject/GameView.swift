...
struct GameView: View {
    ...
    var body: some View {
        RealityView { content in
            // 1. ANCORAGGIO "VOLUME-CENTRIC" (Il cuore della richiesta)
            let anchor = Entity()
            
            // Posizioniamo l'ancora in basso (Y = -0.45) e al centro (X, Z = 0)
            // In questo modo, il gioco è allineato col pavimento della finestra dell'app.
            anchor.position = [0, -0.45, 0]
            
            // 2. SCATOLA GIGANTE INVISIBILE (Per i click ovunque)
            ...
            // Posizione del testo relativa all'ancora
            textEntity.position = [-0.2, 0.8, -0.5]
        ...
        .onReceive(timer) { _ in
            ...
            // Rimbalzo (limite di 0.6 metri dal centro)
    ...
    func createBase(on anchor: Entity) {
        ...
        // Fisica statica: la base è solida ma immobile
    ...
    func spawnNewBlock() {
        ...
        // Rotazione direzioni: NORD -> EST -> SUD -> OVEST
        ...
        // Solo collisioni per ora, niente fisica dinamica
    ...
    func placeBlock() {
        ...
        // --- LOGICA DI TAGLIO (Semplificata: niente detriti) ---
        ...
        // Controllo "Troppo Piccolo"
        if newWidth < 0.02 || newDepth < 0.02 {
            gameOverVisuals()
            return
        }
        
        // 1. Aggiorna visivamente il blocco tagliato
    ...
    func gameOverVisuals() {
        if let block = currentBlock as? ModelEntity {
            ...
            // Al Game Over, attiviamo la fisica dinamica sull'ultimo pezzo per farlo cadere (piccolo effetto drammatico)
            block.components.set(PhysicsBodyComponent(mode: .dynamic))
        }
